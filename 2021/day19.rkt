  ...
  (for/or ([first-pos sc-1])
    (for/or ([(sc-2 i) (in-indexed sc-2*)])
      (for/or ([second-pos sc-2])
        (define intersection
          (for/sum ([pos sc-2]
                    #:when (set-member? sc-1 (coord+ (coord- pos second-pos) first-pos)))
            1))
...
(define (part-1 scanners)
  (define sc-1 (list->set (first scanners)))
  (define scanners* (map rotations* (rest scanners)))
  (define beacons (car (find-beacons/offsets sc-1 scanners*)))
  (set-count beacons))

(define (part-2 scanners)
  (define sc-1 (list->set (first scanners)))
  (define scanners* (map rotations* (rest scanners)))
  (define offsets (cdr (find-beacons/offsets sc-1 scanners*)))
  (for*/fold ([max-manh 0])
             ([a offsets]
              [b offsets])
